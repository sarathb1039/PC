...
public class DeleteShellPolicyNote {
	
	public static String sheetname = "DeleteShellPolicyNote";
	Common common = CommonManager.getInstance().getCommon();
	private WebDriver driver = ManagerDriver.getInstance().getWebDriver();
	static Logger logger =Logger.getLogger(sheetname);
	
	public Boolean SCRDeleteShellPolicyNote() throws Exception{
		...
		if(common.WaitForElementExist(Common.o.getObject("eleShellPolicyNoteTitle"),  Integer.valueOf(HTML.properties.getProperty("LONGWAIT"))))
		{
			logger.info("System displayed Shell Policy Notes search Page");  
			HTML.fnInsertResult(PCThreadCache.getInstance().getProperty("testcasename"), PCThreadCache.getInstance().getProperty("methodName"), "System should display Shell Policy Notes search Page","System displayed Shell Policy Notes search Page", "PASS");
			status = true;
		}
		else
		{
			logger.info("System not displayed Shell Policy Notes search Page");
			HTML.fnInsertResult(PCThreadCache.getInstance().getProperty("testcasename"), PCThreadCache.getInstance().getProperty("methodName"), "System should display Shell Policy Notes search Page","System not displayed Shell Policy Notes search Page", "FAIL");
			status = false;
		}
		return status;
	}
