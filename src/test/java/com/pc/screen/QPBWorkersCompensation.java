...
public class QPBWorkersCompensation {
	
	public static String sheetname = "QPBWorkersCompensation";
	static Logger logger =Logger.getLogger(sheetname);
	Common common = CommonManager.getInstance().getCommon();
	private WebDriver driver = ManagerDriver.getInstance().getWebDriver();
	public Boolean SCRQPBWorkersCompensation() throws Exception{
	...
	public boolean VerifyResults(String sFuncValue) throws Exception
	{
		...
		String Value = null;
			...
			{				
				case "VERIFYSICINDUSTRYCODE":
					logger.info(sValue[0]);
					status = common.ElementExistOrNotTrue(sValue[0], "System should display appropriate SIC Industry Code", "System displays appropriate SIC Industry Cdoe", Common.o.getObject("lstQPBSICCodeCheck"));
//					Value = Common.ReadElementGetAttribute(Common.o.getObject("eleQPBSICCodeCheck"), "value", Integer.valueOf(HTML.properties.getProperty("VERYLONGWAIT")));
//					status = Common.CompareStringResult(sValue[0], sValue[1], Value);
					break;
				case "VERIFYQPBSAVING":
					status = common.SafeAction(Common.o.getObject("eleStateCoverages"), "eleStateCoverages", "eleStateCoverages");
...
//				        		  status = Common.SafeAction(Common.o.getObject("eleQPBPageAssert"), "eleQPBPageAssert", "eleQPBPageAssert");
//				        		  Thread.sleep(5000);
//				        		  Common.WaitForPageToBeReady();
				        		  break;
	        	  			}	
...
//				        		  	 Thread.sleep(5000);
	...
	public Boolean QPBCoverageMarketingFlyers1(String strParam1) throws Exception{

		Boolean status = true;
		String[] strPar = strParam1.split(":::::");
		String[] strParamsLabel = strPar[0].split(":::");
		String[] strParamsValue = strPar[1].split(":::");
		int x=0;
         for(String str: strParamsLabel)
         {
		        	  ...
		        	  if(objName.contains(str) )
		        		  ...
		        		  if(cla.contains(clas))
		        		  {
								  ...
								  WebElement  elem = Common.returnObject(By.id(readAttriID1));
				        		  	...
				        		  	//Common.highlightElement(By.id(readAttriID1));
				        		  	elem.clear();
				        		  	elem.sendKeys(strParamsValue[x]);
	...
	public Boolean QPBCoverageMarketingFlyers3(String strParam1) throws Exception
	{
		Boolean status = true;
		String[] strPar = strParam1.split(":::::");
		String[] strParamsLabel = strPar[0].split(":::");
		...
		int x=0;
         for(String str: strParamsLabel)
         {
        	 ...
        	 WebElement  mytable = Common.returnObject(Common.o.getObject("eleQPBCoverageMarketingFlyers"));
				List<WebElement> allrows = mytable.findElements(By.tagName("tr"));
				  for(int i = 0; i <= allrows.size()-1; i++)
				  {
					  List<WebElement> Cells = allrows.get(0).findElements(By.tagName("td"));
					  String readText = Cells.get(0).getText();
					  if (readText.contains(str))
					  {
						  List<WebElement> CellElements = Cells.get(0).findElements(By.tagName("input"));
				          for(WebElement element: CellElements)
				          {
				        	  element.clear();
				        	  element.sendKeys(strParamsValue[x]);
	...
	public Boolean QPBCoverageMarketingFlyers2(String strParam1) throws Exception{
        	 ...
        	 WebElement  mytable = Common.returnObject(Common.o.getObject("eleQPBCoverageMarketingFlyers"));
				List<WebElement> allrows = mytable.findElements(By.tagName("tr"));
				  for(int i = 0; i <= allrows.size()-1; i++)
				  {
					  ...
					  String readText = Cells.get(i).getText();
					  if (readText.contains(str))
					  {
						  List<WebElement> CellElements = Cells.get(0).findElements(By.tagName("input"));
				          for(WebElement element: CellElements)
...
//		         }
			  ...
			  else
			  {
				  ...
				  HTML.fnInsertResult(PCThreadCache.getInstance().getProperty("testcasename"), PCThreadCache.getInstance().getProperty("methodName"), ""+strParams1[0]+" TestArea should filled",""+strParams1[0]+" TestArea is not filled","PASS");
				  status = false;
			  }
		}
		return status;
...
//		         }
			if(status)
			  ...
			  else
			  {
				  ...
				  HTML.fnInsertResult(PCThreadCache.getInstance().getProperty("testcasename"), PCThreadCache.getInstance().getProperty("methodName"), ""+strParams1[0]+" should filled",""+strParams1[0]+" is not filled","PASS");
				  status = false;
			  }
		}
		return status;
