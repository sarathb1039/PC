...
public class QPBProposalOptions {
	
	public static String sheetname = "QPBProposalOptions";
	static Logger logger =Logger.getLogger(sheetname);
	Common common = CommonManager.getInstance().getCommon();
	private WebDriver driver = ManagerDriver.getInstance().getWebDriver();
	
	public Boolean SCRQPBProposalOptions() throws Exception
	{
	...
	{
		logger.info("Verifying the Results Part");
		boolean status = false;
		String filePath, Value  = null;
	    File file = null;
	    String[] sValue = sFuncValue.split(":::");
		    switch (sValue[0].toUpperCase())
			{
				case "COVERLETTERDOWNLOAD":
					logger.info(sValue[0]);
					common.WaitForPageToBeReady();
						try
						{
						   	  filePath = "C:\\Users\\"+sValue[0]+"\\Downloads\\QuoteProposal.pdf";
							  file = new File(filePath);
							  if (file.exists())
							  {
								  logger.info("The Proposal PDF Generated");
								  HTML.fnInsertResult(PCThreadCache.getInstance().getProperty("testcasename"), PCThreadCache.getInstance().getProperty("methodName"), "The Proposal should be created","The Proposal was successfully created", "PASS");
								  status = true;
							  }
							  else
							  {
								  logger.info("The Proposal PDF not Generated");
								  HTML.fnInsertResult(PCThreadCache.getInstance().getProperty("testcasename"), PCThreadCache.getInstance().getProperty("methodName"), "The Proposal should be created","The Proposal was not created", "FAIL"); 
							  }
						}
						catch(Exception e)
						{
				...
				case "EDITCOVERLETTER":
					logger.info(sValue[0]);
					Value = common.ReadElementGetAttribute(Common.o.getObject("edtQPBCoverLetter"), "value", Integer.valueOf(HTML.properties.getProperty("VERYLONGWAIT")));
