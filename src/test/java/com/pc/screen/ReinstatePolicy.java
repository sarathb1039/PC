/**
 * @ClassPurpose This Class used for the Reinstate Policy
 * @Scriptor Raj
 * @ReviewedBy
 * @ModifiedBy
...
public class ReinstatePolicy {
	...
	public static String sheetname = "ReinstatePolicy";
	static Logger logger =Logger.getLogger(sheetname);
	Common common = CommonManager.getInstance().getCommon();
	private WebDriver driver = ManagerDriver.getInstance().getWebDriver();

	/**
	 * @function Use to perform all the action in the ReinstatePolicy sheet
	 * @return true/false
	 * @throws Exception
	 */
	public Boolean SCRReinstatePolicy() throws Exception
	{
		Boolean status = true;
	...
	**/
	...
	/**
	 * @function Use to give the value of radio button questions
	 * @param sFuncValue
	 * @return true/false
	 * @throws Exception
	 */
	public boolean ReinstateQuestions(String sFuncValue) throws Exception
	{
		boolean status = false;
		String[] sValue = sFuncValue.split(":::");
		status = ActionOnTable(Common.o.getObject("eleRein_Ques"), 0, 1, sValue[0], sValue[1], "input");
//		Common.WaitForPageToBeReady();
		return status;
	}
	
	/**@function use to click the radio button of reinstatement question as per the input
	 * @param obj
	 * @param readCol
	 * @param actionCol
	 * @param strReadString
