...
public class CreatePolicyNote {
	...
	public Boolean SCRCreatePolicyNote() throws Exception{
		...
		if(common.WaitUntilClickable(Common.o.getObject("elePolicySummaryPage"),  Integer.valueOf(HTML.properties.getProperty("VERYLONGWAIT"))))
		{
			logger.info("System displayed Policy Summary page");  
			HTML.fnInsertResult(PCThreadCache.getInstance().getProperty("testcasename"), PCThreadCache.getInstance().getProperty("methodName"), "System should display Policy Summary page","System displayed Policy Summary page", "PASS");
			status = true;
		}
		else
		{
			logger.info("System not displayed Policy Summary page");
			HTML.fnInsertResult(PCThreadCache.getInstance().getProperty("testcasename"), PCThreadCache.getInstance().getProperty("methodName"), "System should display Policy Summary page","System not displayed Policy Summary page", "FAIL");
			status = false;
		}
		return status;
	}
